...
export function Settings({ data, onPreferences, onRestore, onRemoveDemo }: Props) {
  ...
  return <div className="page-stack settings-page">
    ...
    <section className="settings-section panel"><div className="settings-heading"><span>02</span><div><h2>Market data</h2><p>Free sources and explicit Iranian currency handling.</p></div></div><div className="settings-controls">
      ...
      <label className="toggle-row"><span>Enable TSETMC public quotes<small>Iranian prices are Rial per unit; cached values remain if access fails</small></span><input type="checkbox" checked={data.preferences.tsetmcEnabled} onChange={(e) => onPreferences({ tsetmcEnabled: e.target.checked })} /></label>
      ...
      <div className="conversion-note"><strong>Conventional FX cache</strong><span>{data.preferences.fxLastUpdated ? `Updated ${new Intl.DateTimeFormat('en', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(data.preferences.fxLastUpdated))}` : 'Using bundled fallback until first successful refresh'}</span></div>
    </div></section>
    ...
    <section className="settings-section panel"><div className="settings-heading"><span>04</span><div><h2>Optional sync</h2><p>Supabase is isolated from local storage and disabled unless configured.</p></div></div><div className="sync-controls">
      {!syncRef.current.configured ? <p className="sync-empty">Add <code>VITE_SUPABASE_URL</code> and <code>VITE_SUPABASE_ANON_KEY</code> to enable private cross-device sync.</p> : <><div className="sync-login"><input type="email" aria-label="Sync email" autoComplete="email" inputMode="email" maxLength={254} placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} /><input type="password" aria-label="Sync password" autoComplete="current-password" minLength={8} maxLength={128} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} /><button className="secondary-button" disabled={syncBusy || !email || password.length < 8} onClick={() => void syncAction('signin')}>Sign in</button><button className="text-button" disabled={syncBusy || !email || password.length < 8} onClick={() => void syncAction('signup')}>Create account</button></div>{syncReady && <div className="backup-actions"><button className="primary-button" disabled={syncBusy} onClick={() => void syncAction('push')}>Upload local snapshot</button><button className="secondary-button" disabled={syncBusy} onClick={() => void syncAction('pull')}>Download synced snapshot</button></div>}</>}
    </div></section>
  </div>;
}
