...
import type { Currency, WorkspaceData } from '@/lib/types';
...
export default function SettingsPage() {
  ...
  const exportData = () => { const blob = new Blob([JSON.stringify({ assets: store.assets, habits: store.habits, tasks: store.tasks, activities: store.activities, displayName: store.displayName, baseCurrency: store.baseCurrency }, null, 2)], {type:'application/json'}); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `sjd-backup-${new Date().toISOString().slice(0,10)}.json`; link.click(); URL.revokeObjectURL(link.href); setNotice('Backup downloaded.'); };
  const restore = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; try { store.replaceData(JSON.parse(await file.text()) as WorkspaceData); setNotice('Backup restored.'); } catch { setNotice('That backup could not be read.'); } event.target.value=''; };
